"""
@provenweird TikTok Video Pipeline v4

Changes from v3:
- Kling AI replaced with MiniMax Hailuo AI (image-to-video)
- Mixed render: ANIMATED_INDICES scenes get MiniMax, the rest get Ken Burns
- Ken Burns is free (FFmpeg local) — only 4 of 12 clips hit MiniMax API
- Expected cost: ~$0.40-0.60/video (vs ~$9/video with Kling)

MiniMax API flow:
  POST /v1/video_generation          → task_id
  GET  /v1/query/video_generation    → poll for status + file_id
